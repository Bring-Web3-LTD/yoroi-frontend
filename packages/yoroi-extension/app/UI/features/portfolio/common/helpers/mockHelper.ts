...
const getFromTime = (timePeriod: TimePeriodType) => {
  switch (timePeriod) {
    case '24H':
...
export const getRandomTime = (startDate: number) => {
  const date = new Date(startDate + Math.random() * (new Date().getTime() - startDate));
...
export const createChartData = (timePeriod: TimePeriodType) => {
  ...
  const fromTime = getFromTime(timePeriod);
